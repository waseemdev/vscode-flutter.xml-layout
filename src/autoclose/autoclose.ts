        ...
        if (last2chars === "</") {
            ...
            if (closeTag) {
                ...
                editor.edit((editBuilder) => {
                    ...
                    if (nextChar === ">") {
                        (editor as any).selection = moveSelectionRight((editor as any).selection, 1);
                    }
                });
            }
        }
        ...
        if (result !== null && ((occurrenceCount(result[0], "'") % 2 === 0)
            ...
            if (result[2] === ">") {
                if (excludedTags.indexOf(result[1].toLowerCase()) === -1) {
                    editor.edit((editBuilder) => {
                        editBuilder.insert(originalPosition, "</" + (result as any[])[1] + ">");
                    }).then(() => {
                        (editor as any).selection = new vscode.Selection(originalPosition, originalPosition);
                    });
                }
            } else {
